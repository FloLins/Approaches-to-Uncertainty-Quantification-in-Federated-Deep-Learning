...
def run_main():
    ...
    for j in range(10):
        models = list()
        for i in range(MODEL_PARAMETERS['repetitions_for_ensemble']):
    ...
    if CONTROLL_PARAMETERS['plotting']:
        plots.plot_entropy_and_variance(in_data["entropy"], right_wrong["entropy_right"], right_wrong["entropy_wrong"], ood_data["entropy"], in_data["variance"], ood_data["variance"], "Ensemble of Global Models")
        plots.plot_calibration_curve(calibration['curve_label'], calibration['softmax_pred'], "Ensemble of Global Models")
